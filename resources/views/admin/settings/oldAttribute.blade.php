				...
				<div class="panel panel-flat">
					<div class="panel-heading">
						@if($attributeValue->id)
	                      <h5 class="panel-title" ng-init="init('{{$attributeValue->attribute_id}}','{{$attributes}}');">Edit Attribute Value </h5>
	                    @else
                    ...
                    <div class="panel-body">
						@if($attributeValue->id)
                            {!! Form::open(['route' => ['postAttributeValue',$attributeValue->id], 'method' => 'post','id' => 'attributeData','files'=>true]) !!}
                        @else
                            {!! Form::open(['route' => ['postAttributeValue'], 'method' => 'post','id' => 'attributeData','files'=>true]) !!}
                        @endif
                        	<div class="form-group">
                        		<label>Attribute :</label>
                        		<select class="form-control" ng-init="selectAttribute" ng-model="attributes" ng-change="update('{{$attributes}}')" name="attribute_id">
							...
							<div class="form-group">
								...
								<a href="{{ route('attributeValues') }}" class="btn btn-warning">Reset</a>
							</div>
						{{Form::close()}}
					</div>
				</div>
						...
						<table class="table table-striped">
							...
							<tbody>
								...
								<tr>
									...
									<td><a href="{{ route('attributeValues',$attributeValue->id) }}"><i class=" icon-pencil7"></i></a></td>
								</tr>
								@empty
