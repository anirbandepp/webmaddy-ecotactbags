...
<body>
	<div class="container">
        ...
        <table style="border:none;">
		    <tr>
		        <td col="2" style="text-align:center;"><h3><img src="https://www.ecotactbags.com/front-end/logo-green.jpg" loading="lazy" alt="Ecotact Logo"  style="width:170px;"></h3></td>
		    </tr>
		</table>
		<table style="border:none;">
		    <tr>
		        if($order->carrier_partner == 4)
		            <td col="2"><h3>Tracking Url: {{$order->tracking_info}}</h3></td>
		        @elseif($order->carrier_partner == 5)
		            <td col="2"><h3>Tracking Id: {{$order->tracking_info}}</h3></td>
		        @endif
		    </tr>
		</table>
		<table>
			...
			<tbody>
				<tr>
					<td>
						...
						{{$order->country .','. $order->state .','. $order->city .','. $order->pincode }} <br><strong>Phone:</strong> {{$order->contact_no}} </p>
					</td>
					<td>
						<p><strong>{{ $order->name }} <br> {{ $order->email }}</strong><br>
							{{ $order->full_address ? $order->full_address :  $order->house_no .','. $order->apertment_no .','. $order->area .','. $order->landmark}}<br>
		...
		<table class="table-amount">
			...
			<tbody>
				@foreach($order->orderProducts as $orderProduct)
				    @php $curr = $orderProduct->stock->currency @endphp
				<tr>
					<td>
					...
					@if($orderProduct->stock->pieces)
					    <td>{{$orderProduct->stock->currency}} {{$orderProduct->price *  $orderProduct->qty * $orderProduct->stock->pieces}}</td>
					  @else
					    <td>{{$orderProduct->stock->currency}} {{$orderProduct->price * $orderProduct->qty}}</td>
					 @endif
				...
				<tr>
					...
					<td>{{$curr == '₹' ? 'INR' : '$'}} @if($order->discount_amt != $order->gross_amt) {{$order->gross_amt - $order->tax_inclusive}} @else {{$order->net_amt - $order->tax_inclusive}} @endif</td>
				</tr>
				<tr>
					<td></td>
					<td></td>
					...
					<td>@if($order->discount_amt != $order->gross_amt) {{$curr == '₹' ? 'INR' : '$'}} {{$order->discount_amt}} @else  @endif</td>
				</tr>
				@if($curr == '₹')
				<tr>
					<td></td>
					<td></td>
					<td></td>
					<td colspan="1"><strong>GST</strong></td>
					<td> {{$curr == '₹' ? 'INR' : '$'}} {{$order->tax_inclusive}} </td>
				</tr>
				@endif
				<tr>
					<td></td>
					...
					<td>@if($order->discount_amt != $order->gross_amt) {{$curr == '₹' ? 'INR' : '$'}} {{$order->net_amt}} @else  @endif</td>
				</tr>
